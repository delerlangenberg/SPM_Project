...
- Initial `MotionBackend` + `PrusaGcodeBackend` are present
- Prusa MK4S physical integration and calibration are now the active phase

---

## 7. Next Step

Move from software scaffolding to hardware execution:
- connect SPM control host to Prusa MK4S over serial
- validate safe motion commands (home + bounded XY moves)
- wire Z-control and readback into scan workflow
- run first low-speed raster scan with bounded travel limits
