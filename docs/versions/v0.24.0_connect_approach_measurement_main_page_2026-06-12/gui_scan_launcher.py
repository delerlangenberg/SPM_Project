...
# ============================================================
APP_VERSION = "v0.24.0"
APP_PHASE = "Essential SPM Workflow - Connect, Approach, Measurement"
...
class ScanGUI(QWidget):
    ...
    def refresh_workstation_status_ui(self) -> None:
        ...
        if hasattr(self, "main_z_status_label"):
            if self.workstation_status.system_check_passed and self.z_approached:
                self.main_z_status_label.setText("Approach complete. Measurement can start; retract before shutdown.")
                self.main_z_status_label.setStyleSheet("border: 1px solid #1565c0; color: #0d47a1; background: #eef6ff; padding: 8px;")
            elif self.workstation_status.system_check_passed:
                self.main_z_status_label.setText("Approach ready. Set distance above surface, then use manual move or Z AUTO APPROACH.")
    ...
    def run_main_z_auto_approach(self) -> None:
        ...
        self.refresh_workstation_status_ui()
        self.main_z_status_label.setText(f"Approach complete at Z{result.final_z:.2f}. Measurement can start; retract before shutdown.")
        self.main_z_status_label.setStyleSheet("border: 1px solid #1565c0; color: #0d47a1; background: #eef6ff; padding: 8px;")
