...
def test_gui_has_phase_6_1_placeholders():
    ...
    assert "SYSTEM STATUS: NOT CONNECTED" in source
...
def test_system_check_runs_no_motion_hardware_report_before_real_motion():
    ...
    assert "run_workstation_initialization(self.config, self.z_driver)" in system_check_block
    assert "no-motion hardware communication checks" in system_check_block
    assert "if not init_result.passed:" in system_check_block
    assert "INITIALIZATION FAILED - inspect log" in system_check_block
...
def test_gui_locks_actions_until_initialization_passes():
    ...
    assert "Connect to SPM required" in source
    ...
    assert "self.main_z_create_btn.setEnabled(enabled)" in source
    assert "self.main_z_approach_btn.setEnabled(enabled)" in source


def test_initialization_popup_contains_safety_checklist():
    source = Path("core/application/gui_scan_launcher.py").read_text(encoding="utf-8")

    assert 'APP_VERSION = "v0.25.0"' in source
    assert "Focused SPM Workflow - Connection, Approach, Measurement" in source
    assert "Connect to SPM" in source
    assert "SPM connection is starting" in source
    ...
    assert "run_workstation_initialization(self.config, self.z_driver)" in source
    assert "SYSTEM STATUS: SYSTEM READY" in source
...
def test_gui_parks_mk4s_on_button_and_close():
    ...
    assert 'self.power_off_btn = QPushButton("DISCONNECT")' in source
...
def test_gui_has_power_top_level_and_measurement_windows():
    ...
    assert 'QPushButton("CONNECT TO SPM")' in source
    assert 'QPushButton("POWER ON / INITIALIZE")' in source
    assert 'QPushButton("DISCONNECT")' in source
    ...
    assert 'QPushButton("SERVICE APPROACH")' in source
...
def test_gui_main_page_has_only_connection_approach_measurement():
    source = Path("core/application/gui_scan_launcher.py").read_text(encoding="utf-8")
    main_layout_start = source.index("# Main workstation layout")
    main_layout_block = source[main_layout_start:source.index("workstation_page = QWidget()")]
    z_button_start = source.index("z_button_row = QHBoxLayout()")
    z_button_block = source[z_button_start:source.index("main_z_layout.addWidget(self.main_z_status_label)")]

    assert 'main_command_group = QGroupBox("SPM Connection")' in source
    assert 'main_z_group = QGroupBox("Approach")' in source
    assert 'measurement_group = QGroupBox("Measurement")' in source
    assert 'main_layout.addWidget(self.system_status_label)' not in main_layout_block
    assert 'main_layout.addWidget(self.operator_step_label)' not in main_layout_block
    assert "open_hardware_tools_btn" not in main_layout_block
    assert "open_scan_setup_btn" not in main_layout_block
    assert 'self.main_z_approach_btn = QPushButton("Z AUTO APPROACH")' in source
    assert 'self.main_z_retract_btn = QPushButton("RETRACT Z")' in source
    assert 'self.main_z_up_btn = QPushButton("MANUAL UP")' in source
    assert 'self.main_z_down_btn = QPushButton("MANUAL DOWN")' in source
    assert "z_button_row.addWidget(self.main_z_approach_btn)" in z_button_block
    assert "z_button_row.addWidget(self.main_z_retract_btn)" in z_button_block
    assert "z_button_row.addWidget(self.main_z_create_btn)" not in z_button_block
    assert "z_button_row.addWidget(self.main_z_move_btn)" not in z_button_block
    assert "open_z_regulation_btn" not in main_layout_block
    assert "def run_main_z_manual_step" in source
    assert "def run_main_z_auto_approach" in source
    assert "def run_main_z_retract" in source
    assert "setpoint_distance_mm=setpoint_distance" in source


def test_gui_measurement_section_has_xy_start_pause_stop_and_four_topography_views():
    source = Path("core/application/gui_scan_launcher.py").read_text(encoding="utf-8")
    measurement_start = source.index('measurement_group = QGroupBox("Measurement")')
    measurement_block = source[measurement_start:source.index("measurement_group.setLayout")]

    assert 'self.main_scan_btn = QPushButton("START")' in source
    assert "measurement_button_row.addWidget(self.main_scan_btn)" in measurement_block
    assert "measurement_button_row.addWidget(self.pause_scan_btn)" in measurement_block
    assert "measurement_button_row.addWidget(self.stop_scan_btn)" in measurement_block
    assert "xy_parameter_layout.addWidget(self.x_min" in measurement_block
    assert "xy_parameter_layout.addWidget(self.x_max" in measurement_block
    assert "xy_parameter_layout.addWidget(self.y_min" in measurement_block
    assert "xy_parameter_layout.addWidget(self.y_max" in measurement_block
    assert '"X+ TOPOGRAPHY"' in source
    assert '"X- TOPOGRAPHY"' in source
    assert '"Y+ TOPOGRAPHY"' in source
    assert '"Y- TOPOGRAPHY"' in source


def test_gui_no_longer_auto_opens_measurement_window_on_launch():
    source = Path("core/application/gui_scan_launcher.py").read_text(encoding="utf-8")
    main_block = source[source.index('if __name__ == "__main__":'):]

    assert "QTimer.singleShot(500, gui.open_measurement_window)" not in main_block
