...
### Phase M0.5 - Focused Main Workflow Cleanup

Status: complete 2026-06-15.

Reason:

- Operator feedback: the interface still felt like it had several competing Z scanner areas.
- The normal operator workflow should not expose service diagnostics.
- Gwyddion-style lesson: keep the main workspace focused, and move specialist tools into separate modules/windows.

Changes:

- Version updated to `v0.25.0`.
- Phase label updated to `Focused SPM Workflow - Connection, Approach, Measurement`.
- Main page is limited to:
  - `SPM Connection`
  - `Approach`
  - `Measurement`
- Status and next-action text now live inside `SPM Connection`, not as extra standalone blocks.
- The main approach status now says `Approach`, not `Z Scanner`.
- Hidden diagnostic window naming changed from `Z Regulation` to `Service Approach Diagnostics`.
- Service/debug controls remain internal for development, but they are not part of the normal user path.
...
- Focused GUI/Z tests passed: 46 tests.
- Full project test suite passed: 160 tests.
- GUI launched as `Educational SPM v0.25.0 - Operator Workspace - Prusa MK4S`.
- Screenshot saved at `data/v0.25.0_focused_main_workflow_window.png`.
