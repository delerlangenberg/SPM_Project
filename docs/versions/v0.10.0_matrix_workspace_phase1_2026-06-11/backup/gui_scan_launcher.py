...
# ============================================================
APP_VERSION = "v0.10.0"
APP_PHASE = "Phase 1 - MATRIX-style workspace shell"
APP_BUILD_DATE = "2026-06-11"
APP_TITLE = f"Educational SPM {APP_VERSION} - {APP_PHASE} - Prusa MK4S"
...
class ScanGUI(QWidget):
    ...
    def __init__(self):
        ...
        self.open_scan_setup_btn = QPushButton("MEASUREMENT")
        ...
        self.open_hardware_tools_btn = QPushButton("HARDWARE TOOLS")
        self.open_hardware_tools_btn.clicked.connect(self.open_hardware_tools_window)
        self.hardware_check_btn = QPushButton("HARDWARE CHECK")
        ...
        for index, direction in enumerate(("forward", "backward", "upward", "downward")):
            ...
            self.direction_preview_labels[direction] = label
            direction_grid.addWidget(label, index // 2, index % 2)
        ...
        measurement_live_layout.addWidget(self.open_scan_viewer_btn)
        measurement_live_layout.addWidget(feedback_tabs)
        measurement_live_layout.addLayout(direction_grid)
        ...
        self.scan_setup_dialog = self.build_tool_dialog("Measurement Console", measurement_layout, 900, 720)
        ...
        self.hardware_tools_dialog = self.build_tool_dialog("Hardware Tests", hardware_tools_layout, 520, 640)
        ...
        self.main_feedback_label = QLabel("Open MEASUREMENT to view raster, line, topography, and Z approach feedback.")
        ...
        tools_row.addWidget(self.hardware_check_btn)
        tools_row.addWidget(self.open_hardware_tools_btn)
        tools_row.addStretch()

        image_row = QHBoxLayout()
        image_row.addWidget(QLabel("Live image windows:"))
        for direction in ("forward", "backward", "upward", "downward"):
            image_row.addWidget(self.direction_buttons[direction])
        ...
        main_command_layout.addLayout(image_row)
    ...
    def run_main_scan(self) -> None:
        ...
        if mode == "DEMO SOFTWARE":
            self.run_dry_scan(require_initialized=False)
            return
