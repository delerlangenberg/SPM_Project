...
# ============================================================
APP_VERSION = "v0.11.0"
APP_PHASE = "Step 2 - Live SPM raster measurement"
...
class ScanGUI(QWidget):
    ...
    def __init__(self):
        ...
        self.initiate_system_check_btn = QPushButton("POWER ON / INITIALIZE")
        ...
        self.live_line_canvas.setAlignment(Qt.AlignCenter)
        self.live_line_canvas.setStyleSheet("border: 1px solid #607d8b; background: #ffffff;")
        self.live_topography_canvas = QLabel("Topography waiting for raster lines")
        self.live_topography_canvas.setFixedSize(720, 320)
        self.live_topography_canvas.setAlignment(Qt.AlignCenter)
        self.live_topography_canvas.setStyleSheet("border: 1px solid #607d8b; background: #ffffff;")
    ...
    def require_initialized(self, action_name: str) -> bool:
        ...
        QMessageBox.warning(
            ...
            (
                ...
                "Start with INITIATE SYSTEM CHECK. Confirm that the MK4S is powered on, "
    ...
    def refresh_workstation_status_ui(self) -> None:
        ...
        if self.workstation_status.real_motion_enabled:
            ...
            self.system_status_label.setText("SYSTEM STATUS: CHECK PASSED / REAL MOTION DISABLED")
    ...
    def draw_live_line_scan(self) -> None:
        width = self.live_line_canvas.width()
        height = self.live_line_canvas.height()
        ...
        if not line:
            painter.drawText(pixmap.rect(), Qt.AlignCenter, "Line scan waiting for raster data")
            painter.end()
            self.live_line_canvas.setPixmap(pixmap)
            return
        ...
        left, top, right, bottom = 36, 24, width - 18, height - 34
    ...
    def draw_live_topography(self) -> None:
        width = self.live_topography_canvas.width()
        height = self.live_topography_canvas.height()
        pixmap = QPixmap(width, height)
        pixmap.fill(QColor("#ffffff"))
        painter = QPainter(pixmap)
        painter.setRenderHint(QPainter.Antialiasing)
        ...
        if not rows:
            painter.drawText(pixmap.rect(), Qt.AlignCenter, "Topography accumulates one completed line at a time")
            painter.end()
            self.live_topography_canvas.setPixmap(pixmap)
            return
        ...
        high = max(values)
        span = max(1e-9, high - low)
        left, top, right, bottom = 36, 26, width - 18, height - 28
        row_height = max(1, (bottom - top) / max(1, len(rows)))
        for row_index, row in enumerate(rows):
            y0 = top + row_index * row_height
            sample_width = max(1, (right - left) / max(1, len(row)))
            for sample_index, sample in enumerate(row):
                value = float(sample["regulated_height"])
                normalized = (value - low) / span
                color = QColor.fromHsv(int(45 - 42 * normalized) % 360, 220, int(95 + 150 * normalized))
                painter.fillRect(
                    int(left + sample_index * sample_width),
                    int(y0),
                    max(1, int(sample_width) + 1),
                    max(1, int(row_height) + 1),
                    color,
                )
        painter.setPen(QPen(QColor("#263238"), 1))
        painter.drawText(14, 18, f"Topography {selected}: {len(rows)} lines | Z correction {low:.3f}..{high:.3f}")
        painter.end()
        self.live_topography_canvas.setPixmap(pixmap)
    ...
    def initiate_system_check(self) -> None:
        ...
        self.append_log("[SYSTEM CHECK] Running no-motion hardware communication report")
        report = run_hardware_communication_report(self.config)
        for line in report.summary_lines():
            self.append_log(f"[HARDWARE CHECK] {line}")
        report_position = self.parse_position_from_report(report)
        if report_position:
            self.update_position_display(report_position, "initialization readback")

        if not report.passed:
            self.workstation_status.record_system_check_fail("hardware communication check failed")
            self.refresh_workstation_status_ui()
            self.hardware_status_label.setText("Hardware / System Connection: SYSTEM CHECK FAILED - inspect hardware report")
            self.hardware_status_label.setStyleSheet("font-weight: bold; color: #c62828;")
            self.append_log("[SYSTEM CHECK] Failed: hardware communication check did not pass")
            return
        ...
        self.hardware_status_label.setStyleSheet("font-weight: bold; color: #2e7d32;")
        self.operator_step_label.setText(
            "Current step: initialization passed. Adjust scan area/Z settings, run dry-run, then enable real motion only if the MK4S is clear."
        )
        self.append_log("[SYSTEM CHECK] Scan profile valid")
        self.append_log("[SYSTEM CHECK] Hardware communication report passed or contains only non-blocking warnings")
