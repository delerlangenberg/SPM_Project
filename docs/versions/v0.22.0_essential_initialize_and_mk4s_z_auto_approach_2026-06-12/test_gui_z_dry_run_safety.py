...
def test_gui_locks_actions_until_initialization_passes():
    ...
    assert "self.main_z_approach_btn.setEnabled(enabled and self.z_driver.connected)" in source


def test_initialization_popup_contains_safety_checklist():
    source = Path("core/application/gui_scan_launcher.py").read_text(encoding="utf-8")

    assert 'APP_VERSION = "v0.22.0"' in source
    assert "Essential Workflow - Initialize + MK4S Z Auto Approach" in source
...
def test_gui_promotes_z_scanner_workflow_on_main_window():
    source = Path("core/application/gui_scan_launcher.py").read_text(encoding="utf-8")

    assert "Z Scanner / Approach" in source
    assert "CREATE / CONNECT Z SCANNER" in source
    assert "MANUAL Z MOVE" in source
    assert "Z AUTO APPROACH" in source
    assert "RETRACT Z" in source
