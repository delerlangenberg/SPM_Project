...
from core.system.mk4s_z_auto_approach import run_mk4s_z_auto_approach
...
# ============================================================
APP_VERSION = "v0.22.0"
APP_PHASE = "Essential Workflow - Initialize + MK4S Z Auto Approach"
...
class ScanGUI(QWidget):
    ...
    def __init__(self):
        ...
        self.open_z_regulation_btn = QPushButton("Z REGULATION")
        ...
        for direction, label in (
            ("forward", "FORWARD IMAGE"),
            ("backward", "BACKWARD IMAGE"),
            ("upward", "UPWARD IMAGE"),
            ("downward", "DOWNWARD IMAGE"),
        ):
        ...
        self.main_scan_btn = QPushButton("SCAN")
        ...
        self.operator_step_label = QLabel("Next action: initialize the MK4S connection, then validate and dry-run before real motion.")
        ...
        self.initiate_system_check_btn = QPushButton("INITIALIZE")
        ...
        self.power_off_btn = QPushButton("POWER OFF / SAFE PARK")
        ...
        self.main_z_status_label = QLabel("Z Scanner: locked until initialization")
        ...
        self.main_z_create_btn = QPushButton("CREATE / CONNECT Z SCANNER")
        self.main_z_create_btn.clicked.connect(self.run_main_z_create)
        self.main_z_move_btn = QPushButton("MANUAL Z MOVE")
        ...
        self.z_regulation_dialog = self.build_tool_dialog("Z Regulation", z_regulation_layout, 440, 680)
        ...
        # ------------------------------------------------------------
        main_command_group = QGroupBox("Operator Console")
        ...
        session_row.addWidget(self.close_btn)
        session_row.addStretch()

        tools_row = QHBoxLayout()
        tools_row.addWidget(QLabel("Mode:"))
        tools_row.addWidget(self.operation_mode_dropdown)
        tools_row.addWidget(QLabel("Windows:"))
        tools_row.addWidget(self.open_scan_setup_btn)
        tools_row.addWidget(self.open_xy_scanner_btn)
        tools_row.addWidget(self.open_z_regulation_btn)
        tools_row.addWidget(self.open_hardware_tools_btn)
        tools_row.addStretch()

        main_command_layout.addLayout(version_row)
        main_command_layout.addLayout(session_row)
        main_command_layout.addLayout(tools_row)
        ...
        main_z_group = QGroupBox("Z Scanner / Approach")
        main_z_layout = QVBoxLayout()
        z_button_row = QHBoxLayout()
        z_button_row.addWidget(self.main_z_create_btn)
        z_button_row.addWidget(self.main_z_move_btn)
        ...
        workspace_layout.addWidget(main_status_group, 1)

        main_layout.addWidget(self.system_status_label)
        main_layout.addWidget(self.operator_step_label)
    ...
    def update_operation_mode(self, mode: str) -> None:
        if mode == "DEMO SOFTWARE":
            self.operator_step_label.setText("Mode: DEMO SOFTWARE. Use SCAN to test software flow with no hardware movement.")
            self.main_scan_btn.setText("SCAN DEMO")
            self.main_scan_btn.setEnabled(True)
        elif mode == "HARDWARE TEST":
            self.operator_step_label.setText("Mode: HARDWARE TEST. Use the Hardware Tests window for communication, position, and parking checks.")
            self.main_scan_btn.setText("OPEN HARDWARE TESTS")
            self.main_scan_btn.setEnabled(True)
        else:
            self.operator_step_label.setText("Mode: REAL MEASUREMENT. Initialize once, approach Z, then SCAN real hardware.")
            self.main_scan_btn.setText("SCAN REAL")
            self.main_scan_btn.setEnabled(self.workstation_status.system_check_passed)
    ...
    def apply_scan_mode_preset(self, mode: str, show_limits: bool = True) -> None:
        ...
        self.operator_step_label.setText(
            f"Current step: {mode} loaded. Confirm the sample is clear, adjust XY range and MK4S Z height, then validate and dry-run."
        )
    ...
    def set_initialized_controls_enabled(self, enabled: bool) -> None:
        ...
        self.main_z_approach_btn.setEnabled(enabled and self.z_driver.connected)
        self.main_z_retract_btn.setEnabled(enabled and self.z_driver.connected)
        if enabled and self.z_driver.connected:
            self.main_z_approach_btn.setStyleSheet("font-weight: bold; background-color: #1565c0; color: white; padding: 6px;")
        else:
    ...
    def require_initialized(self, action_name: str) -> bool:
        ...
        QMessageBox.warning(
            ...
            (
                f"{action_name} is locked until initialization is complete.\n\n"
                "Start with INITIALIZE. Confirm that the MK4S is powered on, "
    ...
    def refresh_workstation_status_ui(self) -> None:
        ...
        if self.workstation_status.real_motion_enabled:
            ...
            self.system_status_label.setText("SYSTEM STATUS: SAFE MODE / DRY-RUN DEFAULT")
            self.system_status_label.setStyleSheet("font-weight: bold; background-color: #263238; color: white; padding: 6px;")
        ...
        if hasattr(self, "main_z_status_label"):
            if self.workstation_status.system_check_passed and self.z_driver.connected:
                self.main_z_status_label.setText("Z Scanner: READY - choose manual move, auto approach, or retract.")
                self.main_z_status_label.setStyleSheet("border: 1px solid #2e7d32; color: #1b5e20; background: #e8f5e9; padding: 8px;")
            elif self.workstation_status.system_check_passed:
                self.main_z_status_label.setText("Z Scanner: initialization passed; create/connect Z scanner next.")
                self.main_z_status_label.setStyleSheet("border: 1px solid #f9a825; color: #f57f17; background: #fffde7; padding: 8px;")
            else:
                self.main_z_status_label.setText("Z Scanner: locked until initialization.")
                self.main_z_status_label.setStyleSheet("border: 1px solid #90a4ae; background: #fffde7; padding: 8px;")
    ...
    def run_main_z_create(self) -> None:
        if not self.require_initialized("Z scanner create/connect"):
            return
        if self.z_driver.connected:
            self.main_z_status_label.setText("Z Scanner: already ready.")
            self.append_log("[Z SCANNER] Create/connect skipped: already connected")
            return
        self.run_z_dry_connect()
        self.refresh_workstation_status_ui()
        self.append_log("[Z SCANNER] Main workflow create/connect complete")
    ...
    def run_main_z_move(self) -> None:
        ...
        self.refresh_workstation_status_ui()
        self.append_log("[Z SCANNER] Main workflow manual move complete")
    ...
    def run_main_z_auto_approach(self) -> None:
        ...
        reference_text = (
            ...
            "- step Z down to the confirmed foam reference Z56\n\n"
        ...
        if not self.confirm_critical_action("Confirm Z auto approach", reference_text):
            self.append_log("[Z AUTO APPROACH] Cancelled by operator")
            return
        self.main_z_status_label.setText("Z Scanner: running MK4S Z auto approach...")
        self.main_z_status_label.setStyleSheet("border: 1px solid #1565c0; color: #0d47a1; background: #eef6ff; padding: 8px;")
        QApplication.processEvents()
        try:
            result = run_mk4s_z_auto_approach(execute=True, retract_after=False)
        except Exception as error:
            self.main_z_status_label.setText(f"Z Scanner: auto approach failed - {error}")
        ...
        self.refresh_workstation_status_ui()
        self.main_z_status_label.setText("Z Scanner: approached at Z56. Next safe action is RETRACT Z before XY scan or shutdown.")
        self.main_z_status_label.setStyleSheet("border: 1px solid #1565c0; color: #0d47a1; background: #eef6ff; padding: 8px;")
    ...
    def run_main_z_retract(self) -> None:
        ...
        if not self.z_driver.connected:
            self.run_main_z_create()
        if self.z_approached:
            self.z_retract_start.setText(self.z_approach_target.text())
        self.run_z_dry_retract()
        self.refresh_workstation_status_ui()
        self.append_log("[Z SCANNER] Main workflow retract complete")
    ...
    def initiate_system_check(self) -> None:
        ...
        self.append_log("[INITIALIZE] Starting one-button workstation initialization")
        self.initiate_system_check_btn.setEnabled(False)
        self.initiate_system_check_btn.setText("INITIALIZING...")
        QApplication.processEvents()
        ...
        if profile is None:
            ...
            self.initiate_system_check_btn.setText("INITIALIZE")
            self.initiate_system_check_btn.setEnabled(True)
            self.append_log("[INITIALIZE] Failed: scan profile is invalid")
            return
        ...
        try:
            ...
            self.initiate_system_check_btn.setText("INITIALIZE")
            self.initiate_system_check_btn.setEnabled(True)
            self.append_log(f"[INITIALIZE] Failed: {error}")
            QMessageBox.critical(self, "Initialization failed", str(error))
            return
        ...
        if not init_result.passed:
            ...
            self.initiate_system_check_btn.setText("INITIALIZE")
            self.initiate_system_check_btn.setEnabled(True)
            QMessageBox.warning(self, "Initialization incomplete", init_result.recommendation)
            return
        ...
        self.hardware_status_label.setStyleSheet("font-weight: bold; color: #2e7d32;")
        self.initiate_system_check_btn.setText("SYSTEM READY")
        self.initiate_system_check_btn.setEnabled(False)
        self.refresh_z_driver_status("Z scanner ready")
        self.operator_step_label.setText(
            "Current step: SYSTEM READY. Use Z Scanner controls next: manual move, auto approach, or retract."
        )
        self.append_log("[INITIALIZE] Scan profile valid")
        self.append_log("[INITIALIZE] MK4S no-motion checks passed")
        self.append_log("[INITIALIZE] Z scanner dry-run layer created and connected")
        self.append_log("[INITIALIZE] Real motion remains disabled until operator enables it")
