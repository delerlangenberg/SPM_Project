...
APP_VERSION = "v0.19.0"
APP_PHASE = "HT3.3 Guided Hardware Connection Check"
...
class HardwareTestConsole(QWidget):
    def __init__(self) -> None:
        ...
        self.setLayout(root)
        self.append_log(f"{APP_VERSION} loaded. Read-only checks are safe; motion is still locked by default.")
    ...
    def build_header(self) -> QGroupBox:
        ...
        safety = QLabel(
            "Layer 1 hardware-test interface. Start with real read-only connection checks, then unlock supervised "
            "motion only when the MK4S is clear and watched."
        )
    ...
    def update_position_status(self, response_lines: list[str]) -> None:
        ...
        self.position_status.setText(f"XYZ position: X={x_value} mm | Y={y_value} mm | Z={z_value} mm")
