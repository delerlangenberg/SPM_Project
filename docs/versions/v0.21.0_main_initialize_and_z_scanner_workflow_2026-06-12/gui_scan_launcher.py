...
# ============================================================
APP_VERSION = "v0.21.0"
APP_PHASE = "Main Workflow - One Button Initialize + Z Scanner"
...
class ScanGUI(QWidget):
    ...
    def __init__(self):
        ...
        self.main_z_approach_btn = QPushButton("AUTO APPROACH")
    ...
    def run_main_z_auto_approach(self) -> None:
        ...
        if not self.z_driver.connected:
            self.run_main_z_create()
        self.run_z_dry_approach()
        self.refresh_workstation_status_ui()
        if self.z_approached:
            self.z_retract_start.setText(self.z_approach_target.text())
            self.main_z_status_label.setText("Z Scanner: approached. Next safe action is RETRACT Z before XY scan or shutdown.")
            self.main_z_status_label.setStyleSheet("border: 1px solid #1565c0; color: #0d47a1; background: #eef6ff; padding: 8px;")
        self.append_log("[Z SCANNER] Main workflow auto approach complete")
...
if __name__ == "__main__":
    ...
    gui.show()
    QTimer.singleShot(500, gui.open_measurement_window)
    sys.exit(app.exec_())
