...
def test_initialization_popup_contains_safety_checklist():
    ...
    assert 'APP_VERSION = "v0.23.0"' in source
    assert "Essential SPM Workflow - Single Z Scanner" in source
...
def test_gui_has_power_top_level_and_measurement_windows():
    ...
    assert 'QPushButton("INITIALIZE")' in source
...
def test_gui_shows_single_z_scanner_workflow_on_main_window():
    source = Path("core/application/gui_scan_launcher.py").read_text(encoding="utf-8")
    ...
    assert 'main_z_group = QGroupBox("Z Scanner")' in source
    ...
    assert "def run_main_z_create" in source
    assert "def run_main_z_move" in source
    assert "def run_main_z_auto_approach" in source
    assert "def run_main_z_retract" in source
    assert "run_mk4s_z_auto_approach(execute=True, retract_after=False)" in source
