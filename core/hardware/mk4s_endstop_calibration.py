...
def run_home_and_verify(port: str | None = None) -> dict[str, Any]:
    ...
    with Serial(chosen, 115200, timeout=0.2, write_timeout=1.0) as ser:
        ...
        g28_lines = _send(ser, "G28", timeout=120.0)
        log.extend(g28_lines)
        log.extend(_send(ser, "M400", timeout=90.0))
        m119_lines = _send(ser, "M119", timeout=6.0)
        log.extend(m119_lines)
        m119_after = _parse_endstops(m119_lines)
        log.append(f"Endstop state after G28: {json.dumps(m119_after)}")
        m114_lines = _send(ser, "M114", timeout=6.0)
        log.extend(m114_lines)
        home_counts = _parse_counts(m114_lines)
        ...
        g28_ok = any("ok" in line.lower() for line in g28_lines) and ("TIMEOUT" not in "\n".join(g28_lines))
        homing_verified = bool(g28_ok and home_counts is not None)
        log.append(f"Homing verified: {homing_verified}")
        ...
        steering = {
            ...
            "ok": homing_verified,
            "connected": True,
            "ready": homing_verified,
            "status": "calibrated" if homing_verified else "failed",
            "message": "MK4S calibrated and homed successfully. Safe Z park verified." if homing_verified else "MK4S homing did not verify.",
            "home_counts": home_counts,
            "home_mm": home_mm,
            "endstops_before": m119_before,
            "endstops_after": m119_after,
            "all_endstops_triggered": homing_verified,
            "log_lines": log,
