...
from core.ai.academic_ai_client import build_ai_recommendation, get_local_ai_status
...
PHASE_MAP: list[dict[str, str]] = [
    ...
    {
        "phase": "2.2",
        "name": "Z scanner / local LLM advisory layer",
        "status": "implemented",
...
class OperatorConsoleHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if _spm_path == "/api/safety/status":
            from core.system.safety_supervisor import SAFETY_SUPERVISOR
            _spm_send_json(SAFETY_SUPERVISOR.snapshot())
            return

        if _spm_path == "/api/experiment/simulate":
            from core.system.experiment_run_service import ExperimentRunService

            fault_mode = _spm_first("fault", "none")
            run_id = f"console_demo_{fault_mode}"
            try:
                result = ExperimentRunService(PROJECT_ROOT / "data" / "experiment_runs").execute(
                    run_id=run_id,
                ...
                manifest = json.loads(
                    (PROJECT_ROOT / "data" / "experiment_runs" / run_id / "manifest.json").read_text(encoding="utf-8")
                )
                _spm_send_json({
                    "ok": True,
                    "reused": True,
                    ...
                    "fault_mode": manifest["fault_mode"],
                    "artifacts": manifest["artifacts"],
                    "safety": manifest["safety"],
                })
            except ValueError as error:
                _spm_send_json({"ok": False, "status": "not_created", "message": str(error)}, 409)
            return
