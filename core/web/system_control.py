    ...
    if selected_mode in {"hardware_readonly", "real_hardware_readonly", "readonly"}:
        if _spm_os.environ.get("SPM_WEB_ALLOW_READONLY_HARDWARE") != "1":
            return _blocked_payload(
                ...
                "Hardware read-only access is disabled. Enable SPM_WEB_ALLOW_READONLY_HARDWARE=1 for the desktop operator.",
            )
...
def system_safe_retract():
    ...
    if _SPM_SYSTEM_STATE.get("mode") == "real_hardware_readonly" and SAFETY_SUPERVISOR.real_motion_permitted():
...
def system_safe_standby():
    ...
    if not SAFETY_SUPERVISOR.real_motion_permitted():
        return {
            **_base_payload(),
            "ok": False,
            "status": "blocked",
            "mode": "safe_standby_motion_locked",
            "message": "Safe Standby motion is disabled by the Phase 1 safety supervisor.",
            "log_lines": ["BLOCKED: Phase 1 safety supervisor denies real motion."],
        }
    ...
    if not SAFETY_SUPERVISOR.real_motion_permitted():
        return {
            **_base_payload(),
            ...
            "message": "Safe close standby motion is disabled by the Phase 1 safety supervisor.",
            "log_lines": ["BLOCKED: Phase 1 safety supervisor denies real motion."],
        }
...
def system_disconnect():
    if _SPM_SYSTEM_STATE.get("connected") and _SPM_SYSTEM_STATE.get("mode") == "real_hardware_readonly":
        payload = {
            **_base_payload(),
            "ok": True,
            "available": True,
            ...
            "status": "disconnected",
            "mode": "dry_run",
            "message": "Read-only hardware session released without movement.",
            "dry_run_plan": [],
            "log_lines": ["Read-only hardware session released without movement; Z position was not changed."],
        }
        _set_state(payload)
        return payload
...
def system_health_test(confirmed: str = "0", motion: str = "0", profile: str = "short"):
    ...
    if str(motion).strip() == "1":
        return {
            "ok": False,
            "status": "blocked",
            "mode": "health_motion_disabled",
            "message": "Health motion is disabled by safety supervisor: automatic health motion is permanently blocked.",
            "log_lines": ["BLOCKED: Safety supervisor permanently disables automatic health motion."],
        }
