...
def choose_prusa_port(logger: HardwareDevLogger | None = None) -> str:
    ...
    for p in ports:
        identity = f"{p.get('description', '')} {p.get('hwid', '')}".lower()
        if p.get("device") and any(token in identity for token in ("prusa", "mk4", "original prusa")):
            if logger:
                logger.emit(
                    "PASS",
                    "port_selected",
                    reason="matched_prusa_device_identity",
                    device=p.get("device", ""),
                    description=p.get("description", ""),
                    hwid=p.get("hwid", ""),
                )
    ...
    usable_ports = [p for p in ports if p.get("device") and "pnp0501" not in p.get("hwid", "").lower()]
    if len(usable_ports) == 1:
        selected = usable_ports[0]
        if logger:
            logger.emit(
                "WARN",
                "port_selected",
                reason="single_available_serial_port",
                device=selected.get("device", ""),
                description=selected.get("description", ""),
                hwid=selected.get("hwid", ""),
            )
        return selected.get("device", "")
