    ...
    *,
    ...
    if not real_scan_allowed():
        return {
            ...
            "message": "Real scan is disabled by the Phase 1 safety supervisor.",
            "plan": real_scan_plan(profile),
            "log_lines": ["REAL SCAN BLOCKED: Phase 1 safety supervisor denies real motion."],
        }
    ...
    *,
    ...
    if not real_scan_allowed() or not foil_tap_allowed():
        return {
            "ok": False,
            "status": "motion_locked",
            "message": "Foil tap scan is disabled by the Phase 1 safety supervisor.",
            "log_lines": ["FOIL TAP BLOCKED: Phase 1 safety supervisor denies real motion."],
        }
