...
const safetyStateEl = document.getElementById("safety-state");
const safetyDetailEl = document.getElementById("safety-detail");
...
async function loadSafetyStatus() {
  try {
    ...
    safetyStateEl.textContent = `${safety.state} · motion locked`;
    safetyDetailEl.textContent = `Serial: ${safety.serial_opened ? "open" : "closed"} · G-code: ${safety.gcode_sent ? "sent" : "none"}`;
  } catch (error) {
    safetyStateEl.textContent = "Safety status unavailable";
    safetyDetailEl.textContent = "The local safety endpoint is not reachable.";
    log(`Safety status unavailable: ${error.message}`);
  }
}
...
loadStatus();
loadSafetyStatus();
setInterval(loadSafetyStatus, 5000);
if (window.SPMRaster) {
