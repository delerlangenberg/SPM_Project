(function () {
  ...
  function installZPanel() {
    ...
    replaceHeader(panel, "Z Feedback");
  ...
  function installTopographyPanels() {
    ...
    const plus = buildTopographyPanel({
      ...
      title: "Topography X+",
      subtitle: "Accumulated scan image",
    ...
    const minus = buildTopographyPanel({
      ...
      title: "Topography X-",
      subtitle: "Reverse-direction accumulation",
  ...
  function installMeasurementPanel() {
    ...
    replaceHeader(panel, "Measurement Control");
    Array.from(panel.children).forEach((child) => {
      if (!child.classList || !child.classList.contains("panel-header")) child.remove();
    });
    ...
    ["Default Center", "Start Scanning", "Pause", "Stop"].forEach((name) => {
      ...
      b.onclick = () => log(`${name}: staged UI only until XY scanner execution is enabled.`);
  ...
  function installLogPanel() {
    ...
    replaceHeader(panel, "Activity Log");
  ...
  function install() {
    ...
    replaceHeader(qs(".main-panel"), "System Control");
    installZPanel();
    ...
    log("Operator workspace initialized: system, Z feedback, measurement, and activity log.");
  }
