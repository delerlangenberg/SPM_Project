...
## Stage 2 Adaptive Object Map

The header button **Stage 2 Adaptive Object Scan** runs the commissioned CR Touch
feedback workflow. Before selecting it:

1. Confirm the displayed mount is **Centered CR Touch - nozzle removed**.
2. Connect the MK4S and Mega and select **OPERATIONAL** authorization.
3. Fix the objects, verify the complete path, and keep hands clear.
4. Select **High · 2.5 mm focused** for the recommended map.
5. Read the authorization dialog carefully. It must show native X15.50–250.00,
   native Y12.50–210.00, bare-stage trigger Z28.11 and safe travel Z45.

The workflow first searches the full envelope and then concentrates physical
measurements around two detected components. It saves CSV, PNG and JSON evidence
under `data/crtouch_profiles`.

**Research · 1 mm focused** produces far more taps. Use it only while watching
probe temperature and object stability. A smaller pitch does not convert the
CR Touch into a micron-resolution sensor.

### Stage 2 centered-probe safety

The former right-side coordinates are historical and prohibited. The active
profile is `center_crtouch_nozzle_removed`. Real Measurement locks immediately
if D3 is active/latched, heater targets/outputs are nonzero, the mount profile
is incomplete or the mechanical lockout is active.
